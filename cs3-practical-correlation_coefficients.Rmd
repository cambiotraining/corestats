...
```{r cs3-corr-data, message=FALSE, warning=FALSE}
# load the data
...
```{r cs3-state-data, message=FALSE, warning=FALSE}
...
1. The most **positively** correlated variables are  `r USAstate_cor %>% filter(cor == max(cor)) %>% pull(var1)` and `r USAstate_cor %>% filter(cor == max(cor)) %>% pull(var2)`
2. The most **negatively** correlated variables are `r USAstate_cor %>% filter(cor == min(cor)) %>% pull(var1)` and `r USAstate_cor %>% filter(cor == min(cor)) %>% pull(var2)`
3. The most **uncorrelated** variables are `r USAstate_cor %>% filter(cor == min(abs(cor))) %>% pull(var1)` and `r USAstate_cor %>% filter(cor == min(abs(cor))) %>% pull(var2)`
