...
-   The 1st line gives the name of the test and the `data:` tells you which data are used. The `shapiro.test()` function needs a numerical vector as input. We get this by extracting the `length` values with the `pull()` function. We then pipe this to the `shapiro.test()` function.
