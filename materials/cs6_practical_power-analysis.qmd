...
Note: you can download the `pwr_f2_test` script [here](scripts/pwr_f2_test.py) and the `dgplots` script [here](scripts/dgplots.py).
:::
:::
...
* power = `0.8`
* significance = `0.05`
* effect size = `r round(f2, 3)`
* numerator DF = `r round(summary(lm_lobster)$fstatistic[2])`
...
Next, we load the `pwr_f2_test()` function:
