...
The `stats.shapiro()` function requires the numerical input for the two groups it needs to compare.
...
We can do this with the `pd.pivot()` function. We save the output in a new object called `rivers_py_wide` and then access the values as required. It keeps all the data separate, meaning that there will be missing values `NaN` in this format. The `stats.shapiro()` function doesn't ignore missing values by default, so we remove them using `.dropna()`.
...
Perform the Shapiro-Wilk test on the individual groups, omitting the missing values:

```{python}
# perform the Shapiro test
# ignoring the missing values
stats.shapiro(rivers_py_wide['Aripo'].dropna())
stats.shapiro(rivers_py_wide['Guanapo'].dropna())
```
:::
