...
|:------------------|:----------------------------------------------------|
| [`library(tidyverse)`](https://www.tidyverse.org)                             | A collection of R packages designed for data science.                                                             |
| [`library(rstatix)`](https://rpkgs.datanovia.com/rstatix/)                    | Converts base R stats functions to a tidyverse-friendly format. Also contains extra functionality that we'll use. |
| [`library(ggResidpanel)`](https://goodekat.github.io/ggResidpanel/index.html) | Creates diagnostic plots using `ggplot2`.                                                                         |

| Library   | Function| Description|
|:-----------------|:-----------------------|:-----------------------------|
| `rstatix` | [`kruskal_test()`](https://rdrr.io/github/kassambara/rstatix/man/kruskal_test.html) | Performs a Kruskal-Wallis test.                                            |
| `rstatix` | [`dunn_test()`](https://rdrr.io/github/kassambara/rstatix/man/dunn_test.html)       | Performs Dunn's test for pairwise multiple comparisons of the ranked data. |
...
### Post-hoc testing
In this case we should not be doing any post-hoc testing, because we did not detect any statistically significant differences. Doing so anyway and then reporting any incidental groups that _would_ differ, would be p-hacking.
:::
