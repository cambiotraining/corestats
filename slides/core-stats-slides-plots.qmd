...
```{r, alternative depreciated method}
...
# Draw samples and store in a data frame
samples <- replicate(n_repeats, {
  data.frame(value = rnorm(n_samples, mean = mean, sd = sd))
}, simplify = FALSE) %>%
...
```{r, take 100 samples from normal distribution}
set.seed(126)

ngroups <- 100
nres <- 10

normSamp <- data.frame(matrix(rnorm(ngroups*nres, 0, 0.5), nres, ngroups))

normSamp_long <- normSamp %>%
  pivot_longer(X1:X100, names_to="iteration", values_to="value") %>%
  mutate(across(c("iteration"), substr, 2, nchar(iteration))) %>%
  mutate(iteration = factor(iteration, levels=unique(iteration)))

onehundred <- normSamp %>%
  pivot_longer(X1:X100, names_to="iteration", values_to="value") %>%
  mutate(across(c("iteration"), substr, 2, nchar(iteration))) %>%   
  ggplot(aes(x = factor(iteration, levels=unique(iteration)), y = value)) +
    geom_boxplot() +
    labs(x="Sample number", y="log2 fold change", title="Testing one hundred samples") +
    geom_hline(yintercept=0, colour="black", size=0.25, alpha=0.8, linetype=2) + 
    theme_classic(base_size=15) +
...
for(i in 1:ngroups){
  pvals[i]<-t.test(normSamp[,i],mu=0)$p.value
}

onehundredsig <- normSamp %>%
  pivot_longer(X1:X100, names_to="iteration", values_to="value") %>%
  mutate(across(c("iteration"), substr, 2, nchar(iteration))) %>% 
  mutate(sigp = if_else(iteration %in% which(pvals<0.05), TRUE, FALSE)) %>%
  ggplot(aes(x = factor(iteration, levels=unique(iteration)), y = value, fill = sigp)) +
