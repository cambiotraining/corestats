...
|`ggplot2::geom_jitter()`| Plots jittered points by adding a small amount of random variation to each point, to handle overplotting|
|`tidyr::pivot_wider()`| "Widens" the data, increasing the number of columns |

## R

| Function            | Description                                                                                                        |
|:-----------------------------------|:-----------------------------------|
|`stats::reshape`|Reshapes a data frame from 'long' to 'wide' and vice versa|
...
|`scipy.stats.wilcoxon()`|Tests the null hypothesis that two related paired samples come from the same distribution.|
...
All of this suggests that there might be a difference between the legs, with hind legs being longer than forelegs. However, this representation obscures the fact that we have _paired_ data. What we really need to look at is the difference in leg length for each observation:
